...
export default function ContactList() {
  ...
  const fetchListas = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_APP_BACKEND_API}/contact-lists`);
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        <Header pageTitle="Contactos" />

        <main className="p-8">
          <h2 className="text-2xl font-bold mb-4">Crear nueva lista de contactos</h2>
          ...
          <ul className="bg-white p-4 rounded-lg shadow space-y-2">
            {listas.map((lista) => (
              <li key={lista.id} className="border-b pb-2 flex justify-between">
                ...
                <a
                  href={`/settings/contact-lists/${lista.id}/edit`}
